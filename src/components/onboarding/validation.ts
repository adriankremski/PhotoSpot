...
const socialLinksSchema = z
  .record(z.string(), z.string().url('Invalid URL format'))
  .optional()
  .nullable();
...
const baseProfileSchema = z.object({
  display_name: z
    ...
    .max(FIELD_CONSTRAINTS.PROFILE_DISPLAY_NAME_MAX, `Display name must be at most ${FIELD_CONSTRAINTS.PROFILE_DISPLAY_NAME_MAX} characters`),
...
const photographerFieldsSchema = z.object({
  company_name: z
    .string()
    .max(FIELD_CONSTRAINTS.PROFILE_COMPANY_NAME_MAX, `Company name must be at most ${FIELD_CONSTRAINTS.PROFILE_COMPANY_NAME_MAX} characters`)
    .optional()
    .nullable()
    .or(z.literal('')),
