...
export const metadataSchema = z.object({
  ...
  time_of_day: z
    ...
    .optional(),
  tags: z
    .array(z.string().max(FIELD_CONSTRAINTS.TAG_NAME_MAX, `Tag must not exceed ${FIELD_CONSTRAINTS.TAG_NAME_MAX} characters`))
...
const locationSchemaBase = z.object({
  ...
  blur_radius: z.number().min(LOCATION_BLUR.MIN_RADIUS_METERS).max(LOCATION_BLUR.MAX_RADIUS_METERS).default(LOCATION_BLUR.DEFAULT_RADIUS_METERS),
});
  ...
  .refine(
    (data) => {
      if (data.blur_location) {
        return data.blur_radius >= LOCATION_BLUR.MIN_RADIUS_METERS && data.blur_radius <= LOCATION_BLUR.MAX_RADIUS_METERS;
      }
      return true;
    },
    {
