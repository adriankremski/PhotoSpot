...
import { z } from "zod";
import { PAGINATION_DEFAULTS, FILE_UPLOAD_CONSTRAINTS, FIELD_CONSTRAINTS, LOCATION_BLUR } from "../../types";
...
const timeOfDaySchema = z.enum(
  ["golden_hour_morning", "morning", "midday", "afternoon", "golden_hour_evening", "blue_hour", "night"],
  {
  ...
  limit: z
    ...
    .pipe(
      z
        ...
        .max(
          PAGINATION_DEFAULTS.PHOTOS_MAP_VIEW.MAX,
          `Limit must not exceed ${PAGINATION_DEFAULTS.PHOTOS_MAP_VIEW.MAX}`
        )
    )
 ...
 */
export const createPhotoCommandSchema = z
  .object({
...
export const fileValidationSchema = z.object({
  size: z
    .number()
    .max(
      FILE_UPLOAD_CONSTRAINTS.MAX_SIZE_BYTES,
