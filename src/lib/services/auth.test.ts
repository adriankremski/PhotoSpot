...
describe('loginUser', () => {
  ...
  describe('error handling', () => {
    it('should throw 401 error for invalid credentials', async () => {
      ...
      await expect(loginUser('test@example.com', 'wrongpassword', mockClient)).rejects.toThrow(
        AuthServiceError
      );
      await expect(
        loginUser('test@example.com', 'wrongpassword', mockClient)
    ...
    it('should throw 500 error when user data is missing', async () => {
      ...
      await expect(loginUser('test@example.com', 'password123', mockClient)).rejects.toThrow(
        AuthServiceError
      );
      await expect(
        loginUser('test@example.com', 'password123', mockClient)
      ).rejects.toMatchObject({
        code: 'INCOMPLETE_RESPONSE',
        statusCode: 500,
      });
...
describe('requestPasswordReset', () => {
  ...
  describe('successful password reset', () => {
    it('should send password reset email successfully', async () => {
      ...
      await expect(
        requestPasswordReset('test@example.com', mockClient)
  ...
  describe('error handling', () => {
    it('should throw error when reset fails', async () => {
      ...
      await expect(requestPasswordReset('test@example.com', mockClient)).rejects.toThrow(
        AuthServiceError
      );
