...
export async function createUserProfile(
  ...
  try {
    ...
    if (checkError) {
      throw new ProfileServiceError("Failed to check for existing profile", "DATABASE_ERROR", 500, {
        originalError: checkError.message,
      });
    }

    if (existingProfile) {
      throw new ProfileServiceError("Profile already exists for this user", "CONFLICT", 409);
    }
    ...
    if (userError || !userData?.user) {
      throw new ProfileServiceError("Failed to retrieve user information", "USER_NOT_FOUND", 400, {
    ...
    if (!userRole) {
      throw new ProfileServiceError("User role not found in metadata", "INVALID_USER_DATA", 400);
    }

    // Step 3: Validate photographer-only fields against role
    if (userRole !== "photographer") {
      const hasPhotographerFields =
        payload.company_name !== undefined || payload.website_url !== undefined || payload.social_links !== undefined;
    ...
    if (insertError) {
      ...
      if (insertError.code === "23503") {
        throw new ProfileServiceError("User does not exist", "USER_NOT_FOUND", 400, {
          originalError: insertError.message,
        });
      }

      // Generic database error
      throw new ProfileServiceError("Failed to create user profile", "DATABASE_ERROR", 500, {
        originalError: insertError.message,
      });
    }

    if (!createdProfile) {
      throw new ProfileServiceError("Profile creation succeeded but no data was returned", "DATABASE_ERROR", 500);
    }
    ...
    if (!completeProfile) {
      throw new ProfileServiceError("Profile was created but could not be retrieved", "DATABASE_ERROR", 500);
    }
    ...
    throw new ProfileServiceError("An unexpected error occurred while creating user profile", "INTERNAL_ERROR", 500, {
...
export async function updateUserProfile(
  ...
  try {
    ...
    if (Object.keys(updates).length === 0) {
      throw new ProfileServiceError("No valid fields provided for update", "INVALID_INPUT", 400);
    }
    ...
    if (error) {
      ...
      if (error.code === "PGRST116") {
        throw new ProfileServiceError("User profile not found", "NOT_FOUND", 404);
      }

      // Handle constraint violations
      if (error.code === "23505") {
        throw new ProfileServiceError("Profile update failed due to duplicate constraint", "DUPLICATE", 400, {
          originalError: error.message,
        });
      }

      // Generic database error
      throw new ProfileServiceError("Failed to update user profile", "DATABASE_ERROR", 500, {
        originalError: error.message,
      });
    }

    if (!data) {
      throw new ProfileServiceError("Profile update succeeded but no data was returned", "DATABASE_ERROR", 500);
    }
    ...
    if (!updatedProfile) {
      throw new ProfileServiceError("Profile was updated but could not be retrieved", "DATABASE_ERROR", 500);
    }
    ...
    // Wrap unexpected errors
    throw new ProfileServiceError("An unexpected error occurred while updating user profile", "INTERNAL_ERROR", 500, {
      originalError: error instanceof Error ? error.message : String(error),
    });
  }
}
