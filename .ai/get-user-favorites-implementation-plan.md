...
2. **Validation** (`src/lib/validators/favorites.ts`)
   ...
   export const favoriteQueryParamsSchema = z.object({
     limit: z
       .preprocess(Number, z.number().int().min(1).max(PAGINATION_DEFAULTS.FAVORITES.MAX).optional())
       .default(PAGINATION_DEFAULTS.FAVORITES.DEFAULT),
     offset: z.preprocess(Number, z.number().int().min(0).optional()).default(0),
   });
...
4. **API Route** (`src/pages/api/users/[userId]/favorites/index.ts`)
   ...
   export async function GET({ params, request, locals }) {
     try {
       ...
       // 2. Validate query
       const parsed = favoriteQueryParamsSchema.parse(Object.fromEntries(new URL(request.url).searchParams));
       ...
       if (err instanceof ZodError) {
         return new Response(JSON.stringify({ error: { code: "INVALID_REQUEST", message: err.message } }), {
           status: 400,
         });
       }
       // Log & generic 500
